...
class Banks_Model extends CI_Model {
	...
	public function deleteBanks_($table, $params , $index){
		$selected_values = $params[selected_values];  
		$this->db->where("FIND_IN_SET(BankID, '".$selected_values."')");
	...
	function getBankById_($bank_id){   
		$sql = "SELECT * FROM banks_2  
				WHERE FIND_IN_SET(BankID, '".$bank_id."')";//
	...
	public function getBanks_()
	{
		...
		$aColumnsWhere = array('a.BankID', 'a.BankName', 'b.mb_nick', 'a.DateUpdated', 'CASE a.Status WHEN "1" THEN "Active" ELSE "Inactive" END');
		$aColumns = array('a.BankID', 'a.BankName', 'b.mb_nick', 'a.DateUpdated', 'a.Status', 'a.Action');
		
		$sIndexColumn = "a.BankID";
		...
		while ( $aRow = mysql_fetch_array( $rResult ) )
		{
			...
			{
				$edit_link = (admin_access())?base_url().'manage-banks/'.$aRow[BankID]:base_url().'view-bank/'.$aRow[BankID];
				$view_link = base_url().'view-bank/'.$aRow[BankID];
				 
				if( $aColumns[$i] == "a.BankID" ) 
				 {
				 	$row[] = '<input type="checkbox" name="check_user" id="checkbox'.$ctr.'" value="'.$aRow['BankID'].'" >';
				 }  
				elseif ( $aColumns[$i] == "a.BankName" )
				 {  
					$row[] = '<a class="tip" title="manage bank" href="'.$edit_link.'" >'.$aRow[BankName].'</a> ';
				 } 
				...
				elseif ( $aColumns[$i] == "a.Action" )
				 {
					...
					$link = (admin_access())?$link.'<a href="'.$edit_link.'" class="tip" title="edit bank" ><i class="icon16 i-pencil-5 gap-left0 gap-right5"></i></a>':$link;
					$link = (admin_only())?$link.'<a class="tip delete_one" title="delete bank"  bank-id="'.$aRow[BankID].'" ><i class="icon16 i-remove-3 gap-left0 gap-right5 "></i></a>':$link; 
					$row[] = $link; 
				 } 
				else
				 {
