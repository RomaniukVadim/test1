<?php
class Manage_Schedules_Model extends CI_Model {
	...
	function checkTopic_($topic, $topic_id=""){   
		...
		$sql = "SELECT  TopicID 
				...
				WHERE LCASE(Name) =".strtolower($this->db->escape($topic)). $where;//
	...
	function getTopicById_($topic_id){  
		$sql = "SELECT TopicID, Name, Description, SortOrder, AddedBy, UpdatedBy, Status
				FROM {$this->table_name}   
				WHERE FIND_IN_SET(TopicID, ".$this->db->escape($topic_id).")";//
		$result = $this->db->query($sql); 
