...
class Topics_Model extends CI_Model {
	...
	public function deleteTopics_($table, $params , $index){ 
		...
		$this->db->where("FIND_IN_SET(TopicID, '".$selected_values."') $custom_where ");
	...
	function getTopicById_($topic_id){   
		$sql = "SELECT * FROM topics  
				WHERE FIND_IN_SET(TopicID, '".$topic_id."')";//
	...
	public function getTopics_()
	{
		...
		$aColumnsWhere = array('a.TopicID', 'a.Name', 'a.SortOrder', 'b.mb_nick', 'a.DateUpdated', 
							   'CASE a.Status WHEN "1" THEN "Active" ELSE "Inactive" END',  'a.Status'
							  );
		$aColumns = array('a.TopicID', 'a.Name', 'a.SortOrder',  'b.mb_nick', 'a.DateUpdated', 'a.Status', 'a.Action');
		
		$sIndexColumn = "a.TopicID";
		...
		while ( $aRow = mysql_fetch_array( $rResult ) )
		{
			...
			{
				$edit_link = (admin_access())?base_url().'manage-topics/'.$aRow[TopicID]:base_url().'view-topics/'.$aRow[TopicID];
				$view_link = base_url().'view-topics/'.$aRow[TopicID];
				 
				if( $aColumns[$i] == "a.TopicID" ) 
				 {
				 	$row[] = '<input type="checkbox" name="check_user" id="checkbox'.$ctr.'" value="'.$aRow['TopicID'].'" >';
				 }  
				elseif ( $aColumns[$i] == "a.Name" )
				 {  
					$row[] = '<a href="'.$edit_link.'" >'.$aRow[Name].'</a> ';
				 }
				...
				elseif ( $aColumns[$i] == "a.Status" )
				 {
					$row[] = ($aRow[Status]=='1')?"Active":'<span class="red_txt" >Inactive</span>'; 
				 }
				elseif ( $aColumns[$i] == "a.Action" )
				 {
					...
					$link = (admin_access())?$link.'<a href="'.$edit_link.'" class="tip" title="edit topic" ><i class="icon16 i-pencil-5 gap-left0 gap-right5"></i></a>':$link;
					$link = (admin_only() || (admin_access() && $aRow[AddedBy]==$this->session->userdata("mb_no")) )?$link.'<a class="tip delete_one" title="delete topic"  topic-id="'.$aRow[TopicID].'" ><i class="icon16 i-remove-3 gap-left0 gap-right5 "></i></a>':$link; 
					$row[] = $link; 
				 } 
				else  
				 {
