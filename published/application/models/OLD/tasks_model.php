...
class Tasks_Model extends CI_Model {
	...
	public function deleteTasks_($table, $params , $index){
	...
	function getTaskById_($task_id){   
		$sql = "SELECT * FROM tasks  
				WHERE FIND_IN_SET(TaskID, '".$task_id."')";//
	...
	public function getTasks_()
	{
		...
		$aColumnsWhere = array('a.TaskID', 'a.TaskName', 'b.ShiftName', 'a.DateTask', 'CASE a.Status WHEN "1" THEN "Active" ELSE "Inactive" END', 'c.mb_nick', 'a.Status');
		$aColumns = array('a.TaskID', 'a.TaskName', 'b.ShiftName', 'a.DateTask', 'CASE a.Status WHEN "1" THEN "Active" ELSE "Inactive" END', 'c.mb_nick', 'a.Status', 'a.Action');
		
		$sIndexColumn = "a.TaskID";
		...
		if (trim($_GET['sSearch']) != "" )
		{
			...
			{
				$sWhere .= $aColumnsWhere[$i]." LIKE '%".mysql_real_escape_string(trim($_GET['sSearch']))."%' OR ";
			}
		...
		for ( $i=0 ; $i<count($aColumnsWhere) ; $i++ )
		{
			if ( trim($_GET['bSearchable_'.$i]) == "true" && trim($_GET['sSearch_'.$i]) != '' )
			{
				if ( $sWhere == "" )
				{
					$sWhere = "WHERE a.Status<>'9' AND ";
		 ...
		 * Get data to display
		 */
		$sLeftJoin = " LEFT JOIN shift AS b ON a.Shift=b.ShiftID 	 
		...
		while ( $aRow = mysql_fetch_array( $rResult ) )
		{
			...
			{
				$unread_icon = ($aRow[DataTask]==date('Y-m-d'))?'<i class="icon16 i-new gap-left10 red_txt"></i>':''; 
				$edit_link = (admin_access())?base_url().'manage-tasks/'.$aRow[TaskID]:base_url().'view-tasks/'.$aRow[TaskID];
				$view_link = base_url().'view-tasks/'.$aRow[TaskID];
				 
				if( $aColumns[$i] == "a.TaskID" ) 
				 {
				 	$row[] = '<input type="checkbox" name="check_user" id="checkbox'.$ctr.'" value="'.$aRow['TaskID'].'" >';
				 }  
				elseif ( $aColumns[$i] == "a.TaskName" )
				 {  
					$row[] = '<a class="tip" title="manage task" href="'.$edit_link.'" >'.$aRow[TaskName].'</a> '." ".$unread_icon;
				 }
				elseif ( $aColumns[$i] == "b.ShiftName" )
				 { 
					$row[] = $aRow[ShiftName];
				 }
				elseif ( $aColumns[$i] == "a.DateTask" )
				 { 
					$row[] = ($aRow[DateTask] != "0000-00-00")?$aRow[DateTask]:"";
				 }
				...
				elseif ( $aColumns[$i] == "a.Action" )
				 {
					...
					$link = (admin_access())?$link.'<a href="'.$edit_link.'" class="tip" title="edit task" ><i class="icon16 i-pencil-5 gap-left0 gap-right5"></i></a>':$link;
					$link = (admin_only() || (admin_access() && $aRow[AddedBy]==$this->session->userdata("mb_no")) )?$link.'<a class="tip delete_one" title="delete task"  task-id="'.$aRow[TaskID].'" ><i class="icon16 i-remove-3 gap-left0 gap-right5 "></i></a>':$link; 
					$row[] = $link; 
				 } 
				else
				 {
	...
	function searchTask_($keywords="", $where=array(), $action=""){ 
		 
		$aColumnsWhere = array('a.TaskNo', 'a.Remarks', 'a.DateUpdated', 'b.Name', 'c.mb_nick', 'd.Abbreviation'); 
		$sWhere = " a.TaskID<>0 ";  
		if ($keywords != "")
			{
				$sWhere .= " AND (";
		...
		$sql = "SELECT a.*, b.Name AS StatusName, c.mb_nick, d.Abbreviation
				FROM tasks AS a   
				LEFT JOIN task_status AS b ON a.Status=b.StatusID  
				LEFT JOIN g4_member AS c ON a.UpdatedBy=c.mb_no 
