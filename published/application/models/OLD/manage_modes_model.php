<?php
class Manage_Modes_Model extends CI_Model {
	...
	public function __construct()
    {
        ...
        parent::__construct();
		$this->table_name = "agent_modes"; 
    }
	
	function checkModeName_($mode, $mode_id=""){   
		$where = ($mode_id)?" AND ModeID<>".$mode_id." ":"";
		$sql = "SELECT  ModeID 
			  	FROM {$this->table_name} 
				WHERE Status='1' AND LCASE(ModeName) =".strtolower($this->db->escape($mode)). $where;//
		$result = $this->db->query($sql); 
		return $result->num_rows();
	}
	
	function getModeById_($mode_id){  
		$sql = "SELECT a.ModeID, a.ModeName, a.Description, a.UpdatedBy, a.AddedBy,a. UpdatedBy, a.Status 
				FROM {$this->table_name} AS a    
				WHERE FIND_IN_SET(ModeID, ".$this->db->escape($mode_id).")";//
		$result = $this->db->query($sql); 
	...
	public function manageModes_($table, $datax, $action="add", $index="", $index_value=""){ 
		$test="";
		$data = array(); 
