...
class Access extends MY_Controller {
	...
	{   
		...
		$data2 = array("main_page"=>"access",  
					   ...
					   "status_list"=>$this->common->getStatusList_(6),//6 for website/mobile access page  
		...
		$data = array("page_title"=>"12Bet - CAL - Website Mobile Access", 
					  "sidebar_view"=>$this->load->view($sidebar, $data2,true)
					 );
		$this->load->view('header',$data);
		$this->load->view('header_nav');
		$this->load->view('access/access_activities_tpl');
	...
	public function getActivities($actual=0, $action="json", $post_data=array()) 
	{
		...
		if(trim($data[s_tofollowup])==1)
		 {
			...
			$search_url .= "&s_tofollowup=".trim($$data[s_iscomps_tofollowuplaint]);  
			$allow_close++;
		 }
		...
		$return = $activities = $this->access->getAccessActivities_($search_string, $view_statuslist, $paging, $index, $order_by);	  
	...
	{   
		...
		$activity = ($activity_id)?$this->access->getActivity_("csa_website_access", $conditions_array):""; 
		...
		$data2 = array("main_page"=>"access",  
					   "currencies"=>$this->common->getCurrency_(), 
					   "sources"=>$this->common->getSource_(),  
					   "problems"=>$this->access->getProblemById_(array("a.Status ="=>1)),// 
					   "status_list"=>$this->common->getStatusList_(6),//6 for website/mobile access
					   "activity"=>$activity,
					   "s_page"=>$page, 
		...
		$this->load->view('access/access_popup_tpl',$data); 
	...
	{	
		...
		$data['act_usingproxy'] = ($data['act_usingproxy'] == "") ? 0 : $data['act_usingproxy'];  
			...
			if($action == "add")
			 {	 
				...
				$post_data = array(  
					...
					'Problem'=>$data['act_problem'],     
					//'PSP'=>$data['act_psp'],  
					'IPAddress'=>$data['act_ipaddress'],
					'Location'=>$data['act_location'],
					'ISP'=>$data['act_isp'],
					'UsedURL'=>$data['act_usedurl'],
					'UsedBrowser'=>$data['act_usedbrowser'],     
					'IsUsingProxy'=>$data['act_usingproxy'],
					'DeviceUsed'=>$data['act_deviceused'],
					'OSVersion'=>$data['act_osversion'],
					'DateAccessed'=>strtotime($data['act_dateaccessed']),    
					'ErrorMessage'=>$data['act_errormessage'],
				...
				$last_id = $this->access->manageActivity_("csa_website_access", $post_data, $action, '', '');   
				if($last_id > 0)
				 { 
					 //save attacment; 
					 ...
					 $return = array("success"=>1, "message"=>"Website/Mobile Access issue added successfully.", "upload_data"=>json_encode($upload_data), "is_change"=>1);   
					...
					$y = $this->access->manageActivity_("csa_activities_history", $history_data, "add", '', '');  
				 }
				else
				 {
			 ...
			 {   
				...
				if(admin_access() || csd_supervisor_access() || 1){
				 ...
				 $old = $this->access->getActivityById_(array("a.ActivityID ="=>$data["hidden_activityid"]) );	
				 ...
				 $changes .= ($data['act_problem'] != $old->Problem &&($old->Problem!=0))?"Problem changed to ".$data['hidden_aproblem']." from ".$old->ProblemName."|||":"";
				 
				 $changes .= ($old->PSP!=$data['act_psp'])?"Phone Service Provider changed to ".$data['act_psp']." from ".$old->PSP."|||":""; 
				 $changes .= ($old->IPAddress!=$data['act_ipaddress'])?"IP Address changed to ".$data['act_ipaddress']." from ".$old->IPAddress."|||":"";
				 $changes .= ($old->Location!=$data['act_location'])?"Location changed to ".$data['act_location']." from ".$old->Location."|||":"";  
				 
				 $changes .= ($old->ISP!=$data['act_isp'])?"ISP chagend to ".$data['act_isp']." from ".$old->ISP."|||":""; 
				 $changes .= ($old->UsedBrowser!=$data['act_usedbrowser'])?"Used Browser changed to ".$data['act_usedbrowser']." from ".$old->UsedBrowser."|||":""; 
				 $changes .= ($old->UsedURL!=$data['act_usedurl'])?"Used URL changed to ".$data['act_usedurl']." from ".$old->UsedURL."|||":""; 
				 $changes .= ($old->ErrorMessage!=$data['act_errormessage'])?"Error Message changed to ".$data['act_errormessage']." from ".$old->ErrorMessage."|||":""; 
				 ...
				 if($changes != "")
				  {
					$x = $this->access->manageActivity_("csa_website_access", $post_data, $action, "ActivityID", $this->input->post("hidden_activityid"));	 
					if($x > 0)
					 {  
						$return = array("success"=>1, "message"=>"Website/Mobile Access issue updated successfully.", "uploaded_id"=>$uploaded_id, "is_change"=>1);    
						$y = $this->access->manageActivity_("csa_activities_history", $history_data, "add", '', '');  
					 }
					else
					 { 
	...
	{    
		...
		$activity = ($activity_id)?$this->access->getActivityById_($conditions_array):""; 
		...
		$data = array("page_title"=>"12Bet - CAL - Website/Mobile Access Update Status ", 
					  "sidebar_view"=>$this->load->view($sidebar, $data2,true)
					 );
		$this->load->view('access/access_status_popup_tpl',$data); 
		 ...
		 { 
			...
			if($action == "add")
			 ...
			 {   
				 ...
				 $old = $this->access->getActivityById_(array("a.ActivityID ="=>$this->input->post("hidden_activityid")));	
	...
	{    
		...
		$conditions_array = array('a.ActivityID =' => $activity_id); 
		$activity = $this->access->getActivityById_($conditions_array); 
		...
		$this->load->view('access/access_activity_details_popup_tpl', $data); 
	...
	public function exportActivities($actual=0)
	 {	
		...
		foreach($activities as $row=>$activity){   
			...
			$activity->DateAccessed = date("F d, Y H:i:s D", $activity->DateAccessed); 
			...
			$activity->IsUsingProxy = ($activity->IsUsingProxy == '1') ? "YES" : "NO";
	...
	public function exportActivities_XXX($actual=0)
	 {	
		...
		//$page = ($this->uri->segment(3))? $this->uri->segment(3) : 0;
		$page = ($data['s_page'])? $data['s_page'] : 0;
		//$total_rows = $this->access->getCountAccessActivities_($search_string, $view_statuslist, array(), $index, $order_by)->CountActivity;
		//$activities = $this->access->getAccessActivities_($search_string, $view_statuslist, array(), $index, $order_by); 
	 	//$total_rows = count($activities);   
		
		$return = $activities = $this->access->getAccessActivities_($search_string, $view_statuslist, array(), $index, $order_by);	  
		$total_rows = $return[total_rows]; 
		$activities = $return[result];
	...
	{   
		...
		//$this->load->view('header',$data);
		//$this->load->view('header_nav');
		$this->load->view('access/access_users_activities_tpl', $data);
		//$this->load->view('footer');   
	}
		 ...
		 if($error == "")
		  {
			...
			$per_page = 20;  
			$page = ($data['s_page'])? $data['s_page'] : 0;    
			//$total_rows = $this->common->countSearchActivities_($search_string, "csa_website_access", "Username")->CountActivity; 
			//$activities = $this->access->getAccessActivities_($search_string, $view_statuslist, $paging=array("limit"=>$per_page, "page"=>$page), "Username"); 
			
			$return =  $this->access->getAccessActivities_($search_string, $view_statuslist, $paging=array("limit"=>$per_page, "page"=>$page), "Username", "a.DateUpdatedInt");	   			
			$total_rows = $return[total_rows]; 
			$activities = $return[result];
	...
	{ 
		...
		set_time_limit(0);  
		$rows = array();   
		$records = $this->common->getGenerateActivities_("csa_website_access", array("Username <>"=>''), "ActivityID, Username, Currency, AddedBy, DateAdded, UpdatedBy, DateUpdated"); 
		$x = 0;    
