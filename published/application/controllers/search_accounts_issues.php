...
class Search_Accounts_Issues extends MY_Controller {
	...
	{   
		...
		$data2 = array("main_page"=>"accounts",  
					   ...
					   "status_list"=>$this->common->getStatusList_(4, "all"),//1 for bank page 
		...
		$data = array("page_title"=>"12Bet - CAL - Search Account Related Issues", 
					  "sidebar_view"=>$this->load->view($sidebar, $data2,true)
					 );
		$this->load->view('header',$data);
		$this->load->view('header_nav');
		$this->load->view('accounts/account_search_activities_tpl');
	...
	public function getSearchActivities($actual=0)
	{
		...
		$search_string .= " (a.Activity='account_issues') ";  
		$search_string2 = ""; 
		$allow_close = 0;  
		$allow_view = 0;  
		...
		if(trim($data[s_accountproblem]))
		 {
			$search_string2 .= " AND (b.AccountProblem='".$this->common->escapeString_(trim($data[s_accountproblem]))."') "; 
		...
		/*if(trim($data[s_agent]) != "")
		 {
			$search_string2 .= " AND (f.mb_nick LIKE '%".$this->common->escapeString_($data[s_agent])."%') ";  
		...
		//$total_rows = $this->accounts->countActivitiesSearch_($search_string, $search_string2, "csa_account_issues")->CountActivity; 
		//$activities = $this->accounts->getSearchActivities_($search_string, $search_string2, $view_statuslist, $paging=array("limit"=>$per_page, "page"=>$page));
		
		$return = $this->accounts->getSearchActivities_($search_string, $search_string2, $view_statuslist, $paging=array("limit"=>$per_page, "page"=>$page));  
	...
	public function exportSearchActivities($actual=0)
	 {	
		...
		$page = ($data['s_page'])? $data['s_page'] : 0; 
		//$total_rows = $this->accounts->countActivitiesSearch_($search_string, $search_string2, "csa_account_issues")->CountActivity; 
		//$activities = $this->accounts->getSearchActivities_($search_string, $search_string2, $view_statuslist, array());
		//$total_rows = count($activities);    
		
		$return = $this->accounts->getSearchActivities_($search_string, $search_string2, $view_statuslist, array());  
		$total_rows = $return[total_rows]; 
		$activities = $return[result]; 
		...
		$file_name = "search_accounts_issues".'-'.date("Ymdhis").".xls"; 
		$title = "Search Accounts Issues";
