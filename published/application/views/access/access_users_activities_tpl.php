...
function getAccessActivities(){ 
	 
	$.ajax({ 
		data: $("#activities_form").serialize(),
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>access/getUserActivities",  
		beforeSend:function(){   
			//show loading 
			searchLoading("show"); 
		...
		success:function(newdata){    
			...
			$('.activity_row .view_activity').click(function() {
				...
				if(activity_id)
				 {
					 loadAjaxContent("<?=base_url()?>access/viewActivityDetails/"+activity_id, $("#ActivityDetailsModal").find(".ajax_content"));   
				 	 $("#ActivityDetailsModal").find(".modal-title").html("<i class=\"icon20 i-earth\"></i>Website/Mobile Access Issue Details");  	
				 }
			});
