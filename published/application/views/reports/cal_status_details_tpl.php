						...
						<div class="widget-content" >  
                            ...
                            <tbody id="ActivityList" class="dynamic-list" > 
                            	<tr id="SearchRow"  > 
                                    ...
                                    <td class="center" >
                                         <?php /*?><select name="s_currency" class="select2" >
                                            <optgroup label="" >    
                                    ...
                                    <td class="center" >
                                    	<?php /*?><input class="text_filter" name="s_username" type="text" rel="1" value=""><?php */?>    
                                    </td>
                                    ...
                                    <td class="center" >
                                         <select name="s_status" class="select2" style="width: 100px !important;"  >
                                            <optgroup label="" >    
												...
												?>
                                                <option value="<?=$status[StatusID]?>" <?=$selected?> ><?=ucwords($status[Label])?></option>
                                                <?php	
												}//end foreach
												?> 
...
<script id="list-row" type="text/x-handlebars-template">
  {{#if activities.length }}
  	{{#each activities}}	  
	  <tr class="user-row" id="Activity{{this.HistoryID}}" >
	  	 <td class="center" >{{this.CurrencyName}}</td>  
		 <td class="center" >{{displayActivity ../activity_types this.Activity 'Label'}}</td>  
		 ...
		 <td class="left" >{{this.Remarks}}</td> 
		 <td class="center" >{{this.StatusName}}</td>  
		 <td class="center" >{{this.GroupAssignee}}</td> 
		 <td class="center action" >
		 	<a href="#ActivityDetailsModal" title="view activity" alt="view activity" class="view_activity tip" activity="{{displayActivity ../activity_types this.Activity 'Controller'}}" activity-id="{{this.ActivityID}}"  id="View{{this.ActivityID}}{{this.Controller}}"  data-toggle="modal" ><i class="icon16 i-file-8 gap-left0 gap-right10" ></i></a>   
...
function getCalStatusReportsDetails(){ 

	$.ajax({ 
		data: $("#search_form").serialize(),
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>cal/getCalStatusReportsDetails",  
		beforeSend:function(){    
			//show loading 
			searchLoading("show");  
		},
		success:function(newdata){  
			...
			//view details
			$('.user-row .view_activity').click(function() {
				var activity_id = $(this).attr('activity-id');  
				var activity = $(this).attr('activity');  
...
function exportActivities(){
	xhr = $.ajax({ 
		...
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>cal/exportStatusReportsDetails",  
		beforeSend:function(){    
			//show loading  
		 	exportLoading("show");  
		},
...
$(function() { 	 
	...
	$('#ActivityModal, #ActivityStatusModal, #ActivityDetailsModal').on('hide.bs.modal', function (e) {     
		  ...
		  if(is_change == 1)getCalStatusReportsDetails();
		  is_change = 0; //global 
	}); 	 
