					...
					<div class="widget panel">  
						...
						<div class="widget-content panel-body">
							<div class="span12"  >
								...
								<select id="menu" name="menu" class="select2 span9 select2-offscreen" tabindex="-1">
								</select>
							</div>
...
<script>     
  ...
  $(function() {
    $("#market_drop").select2({placeholder: "Select", dropdownCssClass: "market-select-container"});
	$("#menu").select2({placeholder: "Select", dropdownCssClass: "menu-select-container"});
	...
	$("#BtnAdd").click(function(){
	  window.location = "<?=base_url("portal/configure/page/add")?>";
	});
	
  });
  
  function loadMenus(market) {
    $.ajax({
	  ...
	  data: {"cmarket[]": market},
	  error: function() {
	    $("#menu").html("<option value=''>No Record Found</option>");
	  },
	  success: function(response) {
	    if(response.error) {
		  ...
		  $("#menu").select2("val",$('.select2 option:eq(1)').val());
		}
		$("#menu").trigger("change");
	  }
	});
  ...
  function loadPages() {
	$.ajax({
	  ...
	  data: {"market" : $("#market_drop").val(),
	         "menu"   : $("#menu").val()},
	  error: function() {
	    $("#page_table_content").html("<tr><td width=\"100%\" class=\"center\" colspan=\"2\">- No Data -</td></tr>");
	  },
	  success: function(response) {
	    $("#page_table_content").html(response.content);
		$(".delete-btn").click(function(){
		  if(confirm("Are you sure that you want to delete this page?")) {
			deletePage($(this).attr("page-id"));
		  }
		});
	  }
	});
