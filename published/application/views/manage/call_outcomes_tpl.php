        ...
        <div class="crumb">
			<ul class="breadcrumb">
				...
				<li><a href="#">Manange</a><span class="divider">/</span></li>
                  <li class="active">Call Outcomes</li>
			</ul>  
		</div> 
				...
				<div class="span12"> 
					...
					<div class="widget"> 
						...
						<div class="widget-content" >  
                            ...
                            <tbody id="OutcomeList" class="dynamic-list" > 
                            	<tr id="SearchRow"  > 
                                    <td class="center" > 
                                    ...
                                    <td>
                                       <select name="s_result" class="select2"  >
                                            <optgroup label="" >    
                                            	<option value="" >- All -</option>
                                            	<?php   
												foreach($results as $row=>$result) {
												?>
                                                <option value="<?=$result->result_id?>" ><?=ucwords($result->result_name)?></option>
                                                <?php	
												}//end foreach
												?> 
                                            </optgroup>  
							...
							<div class="form-actions"> 
                            	...
                            	<a href="#CallOutcomeModal" class="btn btn_addoutcome"  data-toggle="modal" target-form="call_outcome_form" >
...
<div class="modal fade" id="CallOutcomeModal"  role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" >
	...
	<div class="modal-dialog"  >
	  ...
	  <div class="modal-content"  >
		...
		<div class="modal-header" >
		  ...
		  <h4 class="modal-title"  ><i class="icon20 i-call-outgoing"></i><span>Manage Call Outcome</span></h4>
		</div>
...
<script> 
function getCallOutcomesList(){ 
 	
	$.ajax({ 
		data: $("#search_form").serialize(),
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>call_outcomes/getCallOutcomesList",  
		beforeSend:function(){   
			//show loading 
			searchLoading("show");
		},
		success:function(newdata){   
			...
			searchLoading("hide");
			$("#OutcomeList").find("tr.outcome_row").remove();
			$("#OutcomeList").append(newdata.outcomes);
			...
			$('.outcome_row .edit_outcome').click(function() {   
				$("html, body").animate({ scrollTop: 0 }, "slow"); 
				var outcome_id = $(this).attr('outcome-id');   
				if(outcome_id)loadAjaxContent("<?=base_url()?>call_outcomes/popupManageCallOutcome/"+outcome_id, $("#CallOutcomeModal").find(".ajax_content"));  
			});
...
$(function() { 	 
	...
	$('.btn_addoutcome').click(function(e) {  
		$("html, body").animate({ scrollTop: 0 }, "slow");  
		loadAjaxContent("<?=base_url()?>call_outcomes/popupManageCallOutcome/", $("#CallOutcomeModal").find(".ajax_content")); 
	});
	  
    $('#CallOutcomeModal').on('hide.bs.modal', function (e) {     
		  $(".select2-drop, .select2-drop-mask").hide();  
		  if(is_change == 1)getCallOutcomesList(); 
		  is_change = 0; //global 
	});		 
	
	$('#CallOutcomeModal').modal({ 
		show: false, 
		keyboard: false
	});	 
