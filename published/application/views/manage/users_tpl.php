						...
						<div class="widget-content" >  
                            ...
                            <tbody id="UsersList" class="dynamic-list" > 
                            	<tr id="SearchRow"  > 
                                    ...
                                    <td class="center" >
                                       <input class="text_filter" name="s_nickname" type="text" rel="1" value="" style="text-align: left; " >
                                    </td>
                                            ...
                                            <optgroup label="" >    
												...
												foreach($user_types as $row=>$user_type) {
													if( (admin_only() || admin_access()) || (in_array($user_type->GroupID,$restrict_type) || manage_user()) )
													 {
												?>
                                                <option value="<?=$user_type->GroupID?>" ><?=ucwords($user_type->UserTypeName)?></option>
                                                <?php	
													 }
												}//end foreach
												?> 
                                    ...
                                    <td class="center" >
                                    	<select name="s_currency" class="select2 myselect"  >
                                            <optgroup label="" >    
                                            	<option value="" >- All -</option>
                                            	<?php   
							...
							<div class="form-actions"> 
                            	...
                            	<a href="#UserModal" class="btn btn_addgroup"  data-toggle="modal" target-form="call_outcome_form" >
	...
	<div class="modal-dialog"  >
	  ...
	  <div class="modal-content"  >
		...
		<div class="modal-header" >
		  ...
		  <h4 class="modal-title"  ><i class="icon20 i-users"></i><span>Manage User</span></h4>
		</div>
...
function getUsersList(){ 
	...
	$.ajax({ 
		...
		success:function(newdata){   
			...
			searchLoading("hide");
			$("#UsersList").find("tr.outcome_row").remove();
			$("#UsersList").append(newdata.groups);
			...
			$('.group_row .edit_user').click(function() {   
				$("html, body").animate({ scrollTop: 0 }, "slow"); 
				var user_id = $(this).attr('user-id');   
				if(user_id)loadAjaxContent("<?=base_url()?>users/popupManageUser/"+user_id, $("#UserModal").find(".ajax_content"));  
			});
...
$(function() { 	 
	...
	$('.btn_addgroup').click(function(e) {  
		$("html, body").animate({ scrollTop: 0 }, "slow");  
		loadAjaxContent("<?=base_url()?>users/popupManageUser/", $("#UserModal").find(".ajax_content")); 
	});
	 
	$('#UserModal').on('hide.bs.modal', function (e) {     
		  $(".select2-drop, .select2-drop-mask").hide();  
		  if(is_change == 1)getUsersList();
		  is_change = 0; //global 
	});  
