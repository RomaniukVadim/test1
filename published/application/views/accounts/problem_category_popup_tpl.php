...
<form id="validate_category" name="validate_category" class="form-horizontal"  autocomplete="off" style="margin: 0px; padding: 0px; "  method="post"  onsubmit="return false; " >  
    ...
    <div class="control-group" >   
        ...
        <div class="controls controls-row"  >
            <input type="text" id="category_name" name="category_name" class="tip span12" value="<?=htmlentities(stripslashes($category->Name));?>" maxlength="80" title="Problem Name" > 
        </div>
    </div>
    <!-- End .control-group --> 
   
   	<div class="control-group" >  
        <label class="control-label" for="category_problem">* Account Problem</label>  
        <div class="controls controls-row">  
            <select name="category_problem" id="category_problem" class="required myselect" > 
                <optgroup label="" >    
                     <option value="" <?php if($category->AccountProblem=='') echo "selected='selected'";?> >&nbsp;</option>
                    <?php
                    foreach($problems as $row => $problem){  
					 
                    ?>
                    <option  value="<?=$problem->ProblemID;?>" <?php if($category->AccountProblem==$problem->ProblemID) echo "selected='selected'";?> ><?=ucwords($problem->ProblemName);?></option>
					<?php
                    }
                    ?>
                </optgroup> 
    ...
    <div class="form-actions"> 
        <button type="submit" class="btn btn-primary" id="BtnSubmitForm" ><?=($category->CategoryID)?"Update ":"Save new "?> problem</button>
        &nbsp;&nbsp;
...
<script >
var manageProblemCategory = function() { 
	 
	$.ajax({ 
		data: $("#validate_category").serialize(), 
		type:"POST",  
		url: "<?=base_url();?>related_problem_categories/manageProblemCategory", 
		dataType: "JSON",    
		cache: false,
		beforeSend:function(){       
