        ...
        <div class="crumb">
			<ul class="breadcrumb">
				...
				<li><a href="#">Banks</a><span class="divider">/</span></li>
                  <li class="active">Analysis Categories</li>
			</ul>  
		</div> 
	...
	<div class="modal-dialog"  >
	  ...
	  <div class="modal-content"  >
		...
		<div class="modal-header" >
		  ...
		  <h4 class="modal-title"  ><i class="icon20 i-chart"></i><span>Manage Analysis Category</span></h4>
		</div>
...
function getAnalysisCategories(){ 
  
	$.ajax({ 
		data: $("#search_form").serialize(),
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>analysis_categories/getAnalysisCategories",  
		beforeSend:function(){   
			//show loading
			searchLoading("show");   
		},
		success:function(newdata){    
			...
			$('.activity_row .edit_category').click(function() {   
				...
				if(category_id)loadAjaxContent("<?=base_url()?>analysis_categories/popupManageAnalysisCategory/"+category_id, $("#ReasonModal").find(".ajax_content"));  
			});
...
$(function() { 	 
	...
	$('.btn_addreason').click(function(e) {  
		$("html, body").animate({ scrollTop: 0 }, "slow");  
		loadAjaxContent("<?=base_url()?>analysis_categories/popupManageAnalysisCategory/"+dcategory, $("#ReasonModal").find(".ajax_content")); 
	});
	 
	$('#ReasonModal').on('hide.bs.modal', function (e) {     
		  $(".select2-drop, .select2-drop-mask").hide();  
		  if(is_change == 1)getAnalysisCategories(); 
		  is_change = 0; //global 
	});   
