						...
						<div class="widget-content" >  
                            ...
                            <tbody id="ActivityList" class="dynamic-list" > 
                            	<tr id="SearchRow"  >  
                                    ...
                                    <td class="center" >
                                         <select name="s_method" class="select2"  style="width: 100px !important;" disabled >
                                            <optgroup label="" >    
                                            	<option value="" >- All -</option> 
                                            </optgroup>  
                                        </select>    
...
function getSearchActivities(){ 
	...
	$.ajax({ 
		...
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>search_banks_activities/getSearchActivities",  
		beforeSend:function(){   
			//show loading 
			searchLoading("show");
		},
		success:function(newdata){   
			...
			$(".activity_row .download_attachment").click(function(){
				...
				if(activity_id)window.location.href = "<?=base_url()?>banks/downloadAttachment/"+activity_id+"/deposit_withdrawal";
			});  
			...
			$('.activity_row .view_activity').click(function() {
				var activity_id = $(this).attr('activity-id');  
				if(activity_id)loadAjaxContent("<?=base_url()?>banks/viewActivityDetails/"+activity_id+"/1", $("#ActivityDetailsModal").find(".ajax_content"));   
			});
...
function exportSearchActivities(){
	xhr = $.ajax({ 
		...
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>search_banks_activities/exportSearchActivities",  
		beforeSend:function(){    
			//show loading  
		 	exportLoading("show");  
		},
