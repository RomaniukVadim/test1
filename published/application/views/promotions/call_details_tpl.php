					...
					<div class="widget">
						...
						<div class="widget-content" >  
                            ...
                            <div class="row-fluid"  > 
                            	...
                            	<div class="span6" >
                                	 <label class="radio-inline" for="s_addedby">Agent</label>
                                     <div class="controls controls-row"  >
                                        <select class="select2 myselect span4" name="s_addedby" id="s_addedby"  >
                                        	<optgroup label="" >    
                                                <?php /*?><option value="" >- All Agents -</option> <?php */?>
                                                <?php
												foreach($call_agents as $row=>$agent) {
												?>
...
function getPromotionCallDetails(){ 
	 
	$.ajax({ 
		data: $("#search_form").serialize(),
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>promotions/getPromotionCallDetails",  
		beforeSend:function(){   
			//show loading 
			searchLoading("show");  
		},
...
function exportPromotionsCallDetails(){
 
	xhr = $.ajax({ 
		data: $("#search_form").serialize(),
		type:"POST",  
		dataType: "JSON",
		url: "<?=base_url();?>promotions/exportPromotionsCallDetails",  
		beforeSend:function(){    
			//show loading   
		 	exportLoading("show"); 
		},
...
$(function() { 	 
	...
	$('#ActivityModal, #ActivityStatusModal, #ActivityDetailsModal').on('hide.bs.modal', function (e) {     
		  ...
		  if(is_change == 1)getPromotionCallDetails(); 
		  is_change = 0; //global 
	}); 
	 
	
	$(".btn_export").click(function(){
		exportPromotionsCallDetails();
	});
