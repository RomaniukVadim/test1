...
<form id="validate_category" name="validate_category" class="form-horizontal"  autocomplete="off" style="margin: 0px; padding: 0px; "  method="post"  onsubmit="return false; " >  
    ...
    <div class="control-group" > 
        <label class="control-label" for="category_viewers">Viewers</label>
        <div class="controls controls-row" >  
        <?php   
		$category_viewers = explode(",",$category->Viewers); 
		foreach($groups as $row=>$group) { 
			
		?>
		<label class="radio-inline group-label" >
            <input type="checkbox" value="<?=$group->GroupID?>" name="category_viewer" style="margin-bottom: 5px;" <?=(in_array($group->GroupID,$category_viewers))?"checked='checked'":"";?> /> <?=$group->Name?> 
        </label> 
		<?php	
		}//end foreach
		?>     
        <input type="hidden" value="<?=$category->Viewers;?>" name="category_viewers" id="category_viewers" />
        </div>
    </div>
    <!-- End .control-group -->
    ...
    <div class="control-group" style="padding-bottom: 10px !important;" >  
        ...
        <label class="radio-inline  tip" title="Display in CRM Conversions"  >
            <input type="checkbox" value="1" name="category_conversions" id="category_conversions"  <?=($category->ForConversions=='1')?"checked='checked'":"";?>  /> Display in CRM Conversions 
        </label>
        
        <label class="radio-inline  tip" title="Is claimed only"  >
            <input type="checkbox" value="1" name="category_claimedonly" id="category_claimedonly"  <?=($category->ClaimedOnly=='1')?"checked='checked'":"";?>  /> Claimed Only
        </label>
    </div>
    <!-- End .control-group -->
...
var managePromotionCategory = function() { 
	...
	$.ajax({ 
		...
		type:"POST",  
		url: "<?=base_url();?>promotional_categories/managePromotionCategory", 
		dataType: "JSON",    
		cache: false,
		beforeSend:function(){       
